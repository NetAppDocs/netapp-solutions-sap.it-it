...
+
Se il sistema non ti serve più, esegui la procedura di pulizia seguendo questi passaggi:

+
.. Esegui il flusso di lavoro di eliminazione del clone SnapCenter per il volume di dati SAP HANA, incluso l'arresto del database.
+
[NOTE]
====
Il flusso di lavoro di eliminazione del clone di SnapCenter non elimina la voce  `/etc/fstab` per impostazione predefinita.

Affinché la `/etc/fstab` voce venga eliminata automaticamente durante il processo di eliminazione del clone, il clone deve prima essere ripristinato e reso una risorsa rilevata automaticamente. Se questo passaggio non viene eseguito, la `/etc/fstab` voce non viene eliminata automaticamente e deve essere rimossa manualmente.

====
.. Arrestare i servizi SAP HANA.
.. Esegui il flusso di lavoro di eliminazione del clone SnapCenter per il volume condiviso SAP HANA.
