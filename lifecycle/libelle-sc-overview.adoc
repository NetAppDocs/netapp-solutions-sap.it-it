...
Sebbene i sistemi di riparazione e i sistemi di test per il disaster recovery siano in genere forniti utilizzando cloni di sistema SAP (che non richiedono operazioni di post-elaborazione estese), per i sistemi di test e training aggiornati questi passaggi di post-elaborazione devono essere applicati per consentire la coesistenza con il sistema di origine. Pertanto, questo documento si concentra sugli scenari di refresh del sistema SAP. Maggiori dettagli sui diversi casi d'uso sono disponibili nel report tecnico link:sc-copy-clone-introduction.html["Automazione delle operazioni di copia e clonazione del sistema SAP HANA con SnapCenter"^].
