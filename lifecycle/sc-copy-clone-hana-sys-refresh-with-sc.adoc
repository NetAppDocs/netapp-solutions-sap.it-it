...
|  | Crittografia di persistenza di SAP HANA | SFSS: inizialmente, o se le chiavi radice sono state modificate nel sistema di origine, i backup delle chiavi radice devono essere importati nel sistema di destinazione prima che il ripristino possa essere eseguito. LSS: inizialmente, o se la password del backup delle chiavi radice è stata modificata, la password deve essere impostata nella configurazione LSS. 
...
|  | Crittografia di persistenza di SAP HANA | SFSS: Se un DBID del sistema di origine non esiste già nel sistema di destinazione, il database di sistema deve essere prima ripristinato, prima che il backup della chiave radice di questo tenant possa essere importato. LSS: La topologia LSS verrà automaticamente sincronizzata con la topologia HANA durante il ripristino del database del tenant. 
...
In una configurazione Fibre Channel SAN, è necessario specificare il protocollo utilizzato nella schermata successiva.
