...
Questo documento descrive l'opzione di configurazione SnapCenter più comune per un sistema host singolo SAP HANA MDC con un singolo tenant su FSx per ONTAP. Sono possibili altre opzioni di configurazione e, in alcuni casi, necessarie per specifici sistemi SAP HANA, ad esempio per un sistema host multiplo. Per una descrizione dettagliata delle altre opzioni di configurazione, vedere link:hana-sc-generic-concepts.html["Concetti e Best practice di SnapCenter (netapp.com)"^].
