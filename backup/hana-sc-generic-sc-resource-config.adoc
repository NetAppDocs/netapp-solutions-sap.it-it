...
La configurazione iniziale SnapCenter , come descritto nell'argomento precedente link:hana-sc-generic-initial.html["Configurazione iniziale di SnapCenter"] deve essere eseguita per prima, poiché durante la configurazione delle risorse del database HANA sono richieste credenziali, sistemi di archiviazione e policy. La figura seguente riassume i passaggi e le dipendenze.
