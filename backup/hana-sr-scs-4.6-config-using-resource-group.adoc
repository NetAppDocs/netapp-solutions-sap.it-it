...
È stato creato un utente del database "SnapCenter" per il database di sistema HANA con i privilegi necessari per eseguire operazioni di backup e ripristino (vedere link:hana-sc-generic-scope.html["Backup e ripristino SAP HANA con SnapCenter"^]). È necessario configurare una chiave di archivio utente HANA su entrambi gli host utilizzando l'utente del database sopra indicato.
...
Le operazioni di ripristino e recupero per una configurazione di gruppo di risorse SnapCenter 4.6 sono identiche a quelle di una configurazione di replica non di sistema rilevata automaticamente. Sono disponibili tutte le opzioni di ripristino e recupero automatico. Per ulteriori dettagli, consultare la relazione tecnica link:hana-sc-generic-scope.html["Backup e ripristino SAP HANA con SnapCenter"^].
