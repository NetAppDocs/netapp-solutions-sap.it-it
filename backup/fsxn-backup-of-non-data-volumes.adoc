...
+
Lo scenario standard per i sistemi HANA MDC a tenant singolo è quello di utilizzare risorse complete con ripristino del volume. Per un sistema HANA MDC con più tenant, potrebbe essere opportuno ripristinare solo un singolo tenant. Per ulteriori informazioni sul ripristino del singolo tenant, vedere link:hana-sc-generic-restore.html["Ripristino e ripristino (netapp.com)"^].
