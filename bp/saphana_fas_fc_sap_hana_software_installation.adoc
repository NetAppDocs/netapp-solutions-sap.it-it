...
[role="lead"]
Di seguito sono riportati i requisiti per l'installazione del software SAP HANA.
...
....
+
Utilizzando lo strumento di installazione SAP `hdblcm`, avviare l'installazione eseguendo il comando seguente su uno degli host di lavoro. Utilizzare l' `addhosts`opzione per aggiungere il secondo lavoratore (sapcc-hana-tst-06) e l'host di standby (sapcc-hana-tst-07). La directory in cui è stato memorizzato il file preparato `global.ini` è inclusa con l' `storage_cfg`opzione CLI (`--storage_cfg=/hana/shared`). A seconda della versione del sistema operativo in uso, potrebbe essere necessario installare phyton 2.7 prima di installare il database SAP HANA.
