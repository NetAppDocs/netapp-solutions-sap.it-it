...
La tabella seguente mostra la configurazione LUN di un sistema SAP HANA multiple-host 2+1.

|===
| Scopo | LUN | volume logico LVM 


| Host dati 1 | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| Host dati 2 | FC5_data_mnt00002_1 FC5_data_mnt00002_2 | FC5_data_mnt00002-vol 


| Host di log 1 | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| Host di log 2 | FC5_log_mnt00002_1 FC5_log_mnt00002_2 | FC5_log_mnt00002-vol 
|===

NOTE: Il volume /hana/shared deve essere accessibile da tutti gli host SAP HANA e pertanto viene solitamente esportato tramite NFS. Poiché i sistemi NetApp ASA non supportano NFS come protocollo, NetApp consiglia di utilizzare un sistema AFF o FAS aggiuntivo per il file system /hana/shared.
...
NOTE: Se si utilizza il servizio SAP Local Secure Store (LSS), la  `/lss/shared/SID` directory deve essere accessibile da tutti gli host e viene quindi solitamente esportata tramite NFS. Poiché i sistemi NetApp ASA non supportano NFS come protocollo, NetApp consiglia di utilizzare un sistema AFF o FAS aggiuntivo per il file system /lss/shared.
...
NOTE: Disabilita la protezione ransomware autonoma per questi LUN utilizzando la GUI
...
. Fornire le informazioni richieste:
+
.. il nome dei LUN di log da combinare con LVM, ad esempio  `FC5_log_mnt00001`
.. la quantità di LUN da combinare con LVM, ad esempio 2
...
=== Disattiva la protezione ransomware autonoma per tutti i dati e i LUN di log

Apri il menu contestuale di ogni LUN dati e di log e disabilita la protezione ransomware

image:saphana_asa_fc_image19_5.png["ONTAP System Manager - Panoramica LUN"]
