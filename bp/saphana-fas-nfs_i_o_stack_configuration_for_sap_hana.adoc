...
summary: 'A partire da SAP HANA 1.0 SPS10, SAP ha introdotto i parametri per regolare il comportamento dell"io e ottimizzare il database per i file e i sistemi storage utilizzati.' 
---
...
link:saphana-fas-nfs_sap_hana_installation_preparations_for_nfsv4.html["Precedente: Preparazione dell'installazione di SAP HANA per NFSv4."]
...
....
A partire da SAP HANA 2.0, `hdbparam` è stato obsoleto e i parametri sono stati spostati in `global.ini`. I parametri possono essere impostati utilizzando i comandi SQL o SAP HANA Studio. Per ulteriori informazioni, consulta la nota SAP https://launchpad.support.sap.com/["2399079: Eliminazione di hdbparam in HANA 2"^]. È inoltre possibile impostare i parametri all'interno di global.ini, come illustrato nel seguente testo:
...
…
....
A partire da SAP HANA 2.0 SPS5, la `setParameter.py` è possibile utilizzare lo script per impostare i parametri corretti:
...
....
link:saphana-fas-nfs_sap_hana_data_volume_size.html["Successivo: Dimensione del volume di dati SAP HANA."]
