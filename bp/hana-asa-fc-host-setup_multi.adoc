...
NOTE: I sistemi SAP HANA multi-host richiedono il  `/hana/shared` file system e i  `/lss/shared` file system, nel caso in cui venga utilizzato il servizio SAP Local Secure Store (LSS), connessi a tutti gli host di un sistema. Di solito si tratta di una condivisione NFS fornita da un server NFS. Si consiglia di utilizzare un server NFS ad alta disponibilità, ad esempio un sistema NetApp FAS o AFF. Un'altra opzione è utilizzare il server NFS integrato di un host LINUX per questo.
