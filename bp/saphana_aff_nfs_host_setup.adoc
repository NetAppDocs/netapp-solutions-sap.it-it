...
`echo options nfs max_session_slots=<calculated value> > /etc/modprobe.d/nfsclient.conf` un riavvio. Anche il valore lato server deve essere regolato, impostare il numero di slot di sessione come descritto in link:saphana_aff_nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["Configurazione NFS per NFSv4."]
