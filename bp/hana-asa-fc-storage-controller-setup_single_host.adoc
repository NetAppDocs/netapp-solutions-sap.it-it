...
La tabella seguente mostra la configurazione LUN di un sistema SAP HANA a host singolo.

|===
| Scopo | LUN | volume logico LVM 


| Dati | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| Log | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| Condiviso | FC5_shared | n/a. 
|===

NOTE: Con la configurazione descritta, la directory  `/usr/sap/SID1` in cui è memorizzata la home directory predefinita dell'utente SID1adm si trova sul disco locale. In una configurazione di disaster recovery con replica su disco, NetApp consiglia di creare un LUN aggiuntivo per la directory  `/usr/sap/SID1` in modo che tutti i file system si trovino sulla memoria centrale.


NOTE: Se si utilizza il servizio SAP Local Secure Store (LSS), la `/lss/shared/SID` directory, ovvero la directory di installazione predefinita di LSS, si trova sul disco locale. In una configurazione di disaster recovery con replica su disco, NetApp consiglia di creare un LUN aggiuntivo per la `/lss/shared/SID` directory in modo che tutti i file system siano sulla memoria centrale.
