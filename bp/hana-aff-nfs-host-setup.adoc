...
NOTE: I seguenti esempi mostrano un database SAP HANA con SID=NF2 che utilizza LSS.

Per creare le sottodirectory richieste, eseguire una delle seguenti operazioni:

* Per un sistema a host singolo, montare il `/hana/shared` volume e creare le `shared`, `lss-shared`, e `usr-sap` sottodirectory.
...
....
* Per un sistema multi-host, montare il  `/hana/shared` volume e creare le  `shared`,  `lss-shared` e le  `usr-sap` sottodirectory per ciascun host.
+
...
Il prossimo esempio mostra un database SAP HANA multi-host con SID=NF2 che utilizza NFSv4.1 per i file system di dati e di log e NFSv3 per i  `/hana/shared`,  `/lss/shared`, e  `/usr/sap/NF2` file system. Viene utilizzata una dimensione di trasferimento NFS di 1MB per le letture e 256k per le scritture.
