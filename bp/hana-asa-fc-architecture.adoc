...
Diversi modelli della famiglia di sistemi ASA possono essere combinati e abbinati a livello di storage per consentire la crescita e le diverse esigenze di performance e capacità. Il numero massimo di host SAP HANA che possono essere collegati al sistema storage è definito dai requisiti di performance SAP HANA e dal modello di controller NetApp utilizzato. Il numero di shelf di dischi richiesti è determinato solo dai requisiti di capacità e performance dei sistemi SAP HANA.

La figura seguente mostra una configurazione di esempio con otto host SAP HANA collegati a una coppia ha di storage.

image:saphana_asa_fc_image2a.png["Otto host SAP HANA collegati a una coppia ha dello storage"]
...
image:saphana_asa_fc_image3a.png["Host SAP HANA aggiuntivi collegati a una coppia ha dello storage"]

Indipendentemente dal sistema ASA implementato, il panorama SAP HANA può anche essere scalato aggiungendo qualsiasi storage controller certificato per soddisfare la densità di nodo desiderata, come mostrato nella figura seguente.

image:saphana_asa_fc_image4a.png["Aggiunta di una coppia ha storage aggiuntiva"]
...
Per informazioni dettagliate sulla soluzione di backup e ripristino SAP HANA, consultare link:../backup/hana-sc-vmware-smas-scope.html["Data Protection e high Availability di SAP HANA con SnapCenter, SnapMirror Active Sync e VMware Metro Storage Cluster"^].


NOTE: Al momento della creazione di questo documento, SnapCenter per ASA supporta solo le VM basate su VMware che utilizzano VMDK come storage.
...
image:saphana_asa_fc_image6a.png["Panoramica della replica di SnapMirror Storage"]
