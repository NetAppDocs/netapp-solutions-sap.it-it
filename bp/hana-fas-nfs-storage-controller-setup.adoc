...
NOTE: Se si utilizza il servizio SAP Local Secure Store (LSS), la  `/lss/shared/SID` directory, la directory di installazione predefinita di LSS, si trova sul disco locale. In una configurazione di disaster recovery con replica basata su disco, NetApp consiglia di creare una directory all'interno del  `SID_shared` volume per la  `/lss/shared/SID` directory, in modo che tutti i file system si trovino sulla memoria centrale.
...
| SID_shared | lss-shared | /lss/shared/ | Montato su tutti gli host 
