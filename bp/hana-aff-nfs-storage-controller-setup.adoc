...
Gli snapshot creati da ONTAP Autonomous Ransomware Protection non possono essere utilizzati per il ripristino in avanti del database SAP HANA. Pertanto, non è consigliato utilizzarli per i volumi di dati e di log di SAP HANA e dovrebbero essere disabilitati per questi volumi. Utilizzare la protezione dei dati basata su SnapCenter con Snapshots a prova di manomissione invece per la protezione dal ransomware: link:../backup/hana-sc-generic-scope.html["Backup e recovery SAP HANA con SnapCenter"]. ONTAP Autonomous Ransomware Protection potrebbe essere utile per altri volumi come /hana/shared.

È possibile disabilitare l'attivazione automatica di questa funzionalità durante la creazione di un volume per un ONTAP Cluster, tuttavia si consiglia di mantenere l'impostazione predefinita e di disabilitarla per i volumi in cui questa funzionalità non è necessaria.
...
NOTE: Se si utilizza il servizio SAP Local Secure Store (LSS), la `/lss/shared/SID` directory, ovvero la directory di installazione predefinita di LSS, si trova sul disco locale. In una configurazione di disaster recovery con replica su disco, NetApp consiglia di creare una directory all'interno del `SID_shared` volume per la `/lss/shared/SID` directory, in modo che tutti i file system si trovino sulla memoria centrale.
...
NOTE: Se si utilizza il servizio SAP Local Secure Store (LSS), la `/lss/shared/SID` directory deve essere accessibile da tutti gli host. NetApp consiglia di creare una directory all'interno del `SID_shared` volume per la `/lss/shared/SID` directory.
...
| SID_shared | lss-shared | /lss/shared/SID | Montato su tutti gli host 
...
| Disabilita la protezione ransomware, ad eccezione del volume SID_shared | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state disabled 
