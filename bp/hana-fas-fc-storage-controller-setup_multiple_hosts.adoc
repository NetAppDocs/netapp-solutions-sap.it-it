...
Il `/hana/shared` volume deve essere accessibile da tutti gli host HANA e viene quindi esportato tramite NFS. Sebbene non esistano KPI di prestazioni specifici per il `/hana/shared` file system, NetApp consiglia di utilizzare una connessione Ethernet da 10Gb.
...
NOTE: Se si utilizza il servizio SAP Local Secure Store (LSS), la `/lss/shared/SID` directory deve essere accessibile da tutti gli host. NetApp consiglia di creare un volume per la `/lss/shared/SID` directory e montarlo su tutti gli host utilizzando NFS.
...
Le LUN di un gruppo di volumi LVM devono essere memorizzate in un aggregate diverso e su un controller diverso. La tabella seguente mostra un esempio con due LUN per gruppo di volumi per un sistema SAP HANA multiple host 2+1.
...
NOTE: L'esempio seguente imposta queste opzioni durante la creazione del volume
