...
....
+
Se si utilizza Linux LVM, la configurazione richiesta è diversa. L'esempio seguente mostra un setup di 2+1 host multipli con SID=FC5.
...
NOTE: La directory in cui è stato preparato `global.ini` il file viene memorizzato e incluso con `storage_cfg` Opzione CLI (`-- storage_cfg=/hana/shared`).
...
link:saphana_aff_fc_adding_additional_data_volume_partitions_for_sap_hana_single-host_systems.html["Avanti: Aggiunta di partizioni di volumi di dati aggiuntive per sistemi SAP HANA a host singolo."]
