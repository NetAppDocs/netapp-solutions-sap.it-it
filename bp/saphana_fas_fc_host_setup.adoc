...
link:saphana_fas_fc_sap_hana_storage_connector_api.html["Precedente: API del connettore di storage SAP HANA."]

Prima di configurare l'host, è necessario scaricare NetApp SAN host Utilities da http://mysupport.netapp.com/["Supporto NetApp"^] E installato sui server HANA. La documentazione dell'utility host contiene informazioni sul software aggiuntivo che deve essere installato in base all'HBA FCP utilizzato.

La documentazione contiene anche informazioni sulle configurazioni multipath specifiche per la versione di Linux utilizzata. Questo documento illustra le procedure di configurazione richieste per SLES 15 e Red Hat Enterprise Linux 7.6 o versioni successive, come descritto nella https://library.netapp.com/ecm/ecm_download_file/ECMLP2547958["Guida all'installazione e all'installazione di Linux host Utilities 7.1"^].
...
NOTE: I passaggi da 1 a 6 devono essere eseguiti su tutti gli host worker e standby nella configurazione di più host SAP HANA.
...
....
. Eseguire `multipath -r` Comando per ottenere gli identificatori internazionali (WWID) per i nomi dei file dei dispositivi:
+
...
NOTE: L'output di esempio mostra il contenuto di `/etc/multipath.conf` File, che include nomi alias per le quattro LUN di un sistema a più host 2+1. In caso contrario `multipath.conf` file disponibile, è possibile crearne uno eseguendo il seguente comando: `multipath -T > /etc/multipath.conf`.
...
Questo passaggio è necessario solo se si utilizza LVM. Il seguente esempio riguarda un setup di host 2+1 che utilizza SID FC5.
...
....
. Creare un volume logico per ciascuna partizione di dati e log. Utilizzare una dimensione dello stripe uguale al numero di LUN utilizzati per gruppo di volumi (nell'esempio due) e una dimensione dello stripe di 256k per i dati e 64k per il registro. SAP supporta un solo volume logico per gruppo di volumi.
+
....
...
NOTE: Se i comandi precedenti non trovano i volumi, è necessario riavviare il sistema.
...
link:saphana_fas_fc_i_o_stack_configuration_for_sap_hana.html["Pagina successiva: Configurazione dello stack di i/o per SAP HANA."]
