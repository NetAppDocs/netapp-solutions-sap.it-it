...
Un SSD NVMe (dati) supporta fino a 2/5 host SAP HANA a seconda dei dischi NVMe utilizzati. Gli SSD e gli host SAP HANA devono essere equamente distribuiti tra entrambi i controller di storage. Lo stesso vale per i dischi NVMe interni dei sistemi AFF.


NOTE: L'aggiunta di più shelf di dischi non aumenta la quantità massima di host SAP HANA supportata da uno storage controller.
...
image:saphana_aff_nfs_image9a.png["Figura che mostra la finestra di dialogo input/output o rappresenta il contenuto scritto"]
